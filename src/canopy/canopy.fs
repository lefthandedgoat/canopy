...
(* documented/actions *)
let highlight cssSelector = swallowedJs <| sprintf "document.querySelector('%s').style.border = 'thick solid #ACD372';" cssSelector
...
(* documented/actions *)
let describe text = puts text

(* documented/actions *)
let waitFor2 message f =
    try wait compareTimeout f
    with | :? WebDriverTimeoutException -> raise (CanopyWaitForException(sprintf "%s%swaitFor condition failed to become true in %.1f seconds" message System.Environment.NewLine compareTimeout))
...
let private findByFunction cssSelector timeout waitFunc searchContext reliable =
  if wipMode then colorizeAndSleep cssSelector
  ...
  | :? string as cssSelector ->
    wait elementTimeout (fun _ ->
      elements cssSelector
        |> List.map (fun elem ->
          try writeToElement elem text; true
          with
...
let selected item =
  let selected cssSelector (elem : IWebElement) = if not <| elem.Selected then raise (CanopySelectionFailedExeception(sprintf "element selected failed, %s not selected." cssSelector))
...
(* documented/assertions *)
let deselected item =
  let deselected cssSelector (elem : IWebElement) = if elem.Selected then raise (CanopyDeselectionFailedException(sprintf "element deselected failed, %s selected." cssSelector))
...
(* documented/actions *)
let press key = ((js "return document.activeElement;") :?> IWebElement).SendKeys(key)

//alerts
(* documented/actions *)
let alert() =
  waitFor (fun _ -> browser.SwitchTo().Alert() |> ignore; true)
  browser.SwitchTo().Alert()

(* documented/actions *)
let acceptAlert() = wait compareTimeout (fun _ -> browser.SwitchTo().Alert().Accept(); true)

(* documented/actions *)
let dismissAlert() = wait compareTimeout (fun _ -> browser.SwitchTo().Alert().Dismiss(); true)
...
let ( != ) cssSelector value =
  try wait compareTimeout (fun _ -> (read cssSelector) <> value)
  with
...
(* documented/assertions *)
let ( *= ) cssSelector value =
    try wait compareTimeout (fun _ -> ( cssSelector |> elements |> Seq.exists(fun element -> (textOf element) = value)))
    with
...
(* documented/assertions *)
let ( *!= ) cssSelector value =
  try wait compareTimeout (fun _ -> ( cssSelector |> elements |> Seq.exists(fun element -> (textOf element) = value) |> not))
  with
...
(* documented/assertions *)
let contains (value1 : string) (value2 : string) = if (value2.Contains(value1) <> true) then raise (CanopyContainsFailedException(sprintf "contains check failed.  %s does not contain %s" value2 value1))

(* documented/assertions *)
let notContains (value1 : string) (value2 : string) = if (value2.Contains(value1) = true) then raise (CanopyNotContainsFailedException(sprintf "notContains check failed.  %s does contain %s" value2 value1))

(* documented/assertions *)
let count cssSelector count =
  try wait compareTimeout (fun _ -> (unreliableElements cssSelector).Length = count)
  with
...
let ( =~ ) cssSelector pattern =
  try wait compareTimeout (fun _ -> regexMatch pattern (read cssSelector))
  with
...
(* documented/assertions *)
let ( !=~ ) cssSelector pattern =
  try wait compareTimeout (fun _ -> regexMatch pattern (read cssSelector) |> not)
  with
...
(* documented/assertions *)
let ( *~ ) cssSelector pattern =
  try wait compareTimeout (fun _ -> ( cssSelector |> elements |> Seq.exists(fun element -> regexMatch pattern (textOf element))))
  with
...
(* documented/assertions *)
let is expected actual = if expected <> actual then raise (CanopyEqualityFailedException(sprintf "equality check failed.  expected: %O, got: %O" expected actual))
...
let start b =
  ...
  browser <-
    ...
    | Chrome ->
      ...
      new Chrome.ChromeDriver(chromeDriverService (), options) :> IWebDriver
    | ChromeWithOptions options -> new Chrome.ChromeDriver(chromeDriverService (), options) :> IWebDriver
    | ChromeWithUserAgent userAgent -> chromeWithUserAgent userAgent
    | ChromeWithOptionsAndTimeSpan(options, timeSpan) -> new Chrome.ChromeDriver(chromeDriverService (), options, timeSpan) :> IWebDriver
    | Chromium ->
      let options = Chrome.ChromeOptions()
      options.AddArgument("--disable-extensions")
      options.AddArgument("disable-infobars")
      options.AddArgument("test-type") //https://code.google.com/p/chromedriver/issues/detail?id=799            
      new Chrome.ChromeDriver(chromeDriverService (), options) :> IWebDriver
    | ChromiumWithOptions options -> new Chrome.ChromeDriver(chromeDriverService (), options) :> IWebDriver
...
let on (u: string) =
    try onn u
    with | ex -> if browser.Url.Contains(u) = false then raise (CanopyOnException(sprintf "on check failed, expected expression '%s' got %s" u browser.Url))

(* documented/actions *)
let ( !^ ) (u : string) =
  if browser = null then raise (CanopyOnException "Can't navigate to the given url since the browser is not initialized.")
...
(* documented/actions *)
let waitForElement cssSelector = waitFor (fun _ -> someElement cssSelector |> Option.isSome)
