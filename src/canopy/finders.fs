...
let findByCss (cssSelector : string) (f : (By -> ReadOnlyCollection<IWebElement>)) =
  try f(By.CssSelector(cssSelector)) |> List.ofSeq
  with | ex -> []

let findByXpath xpath f =
  try f(By.XPath(xpath)) |> List.ofSeq
  with | ex -> []

let findByLabel locator f =
  let isInputField (element : IWebElement) = element.TagName = "input" && element.GetAttribute("type") <> "hidden"
  let isField (element : IWebElement) = element.TagName = "select" || element.TagName = "textarea" || isInputField element
...
let findByText text f =
  try f(By.XPath(sprintf """.//*[text() = "%s"]""" text)) |> List.ofSeq
  with | _ -> []

let findByValue value f =
  try findByCss (sprintf """*[value="%s"]""" value) f |> List.ofSeq
  with | _ -> []
...
let findByJQuery jquerySelector f =
    try f(ByJQuery(jquerySelector) :> By) |> List.ofSeq
    with | _ -> []
